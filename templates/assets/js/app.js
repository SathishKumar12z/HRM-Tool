...
function dateFormatting(in_values)
{
...
document.addEventListener("DOMContentLoaded", function () {
	// Get all elements with the class "dateformate"
	var elements = document.querySelectorAll('.dateformate');
	// Loop through each element
	elements.forEach(function (element) {
		// Get the inner text of the current element
		let innvalue = element.innerText;
		// Format the date
...
$('.allow_numbers_only').on('input',function(){
	let entered_value = $(this).val()
	let check_input = isNaN(entered_value)
	if (check_input==true){
		let split_word = entered_value.slice(0,-1)
		$(this).val(split_word)
	}
})
...
//---------------------------->>>>>>> check password in client page in add portion
$(".repassword").on('keyup', function () {
	let add_password = $('#password').val();
	let con_password = $(this).val();
	if (add_password != con_password) {
		$('.client_error').html('<h5 class="text-danger">Incorrect Password</h5>')
	} else {
		$('.client_error').html('')
	}
})

//===================================>>>>> provident fund status bar 
$(".current_status").on('click',function(){
	let c_value = $(this);
	let data = c_value[0].innerText
	let ids = c_value[0].attributes[1].nodeValue
	$.get('/HrmTool/HR/status_record/'+data+'/'+ids,function(){
		location.reload();
	})
})
